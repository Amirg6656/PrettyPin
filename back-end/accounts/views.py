...
def validate_otp_limit(phone_number):
...
class send_otp_view(APIView):
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            if validate_otp_limit(phone_number=phone_number) == 'false':
...
class otp_status_view(APIView):
    ...
    def get(self, request):
        otp_id = OtpIdSerializer(data=request.data).validated_data.get('otp_id')

        try:
            otp = OTP.objects.get(public_id=otp_id)
        except OTP.DoesNotExist:
    ...
    # resend OTP 
    def post(self, request):
        serializer = OtpIdSerializer(data=request.data)
        if serializer.is_valid():
            otp_id = serializer.validated_data['otp_id']

            try:
                phone_number = OTP.objects.filter(public_id=otp_id).first().phone_number
            except OTP.DoesNotExist:
...
class verify_otp_view(APIView):
    ...
    def post(self, request, status=None):
        ...
        if serializer.is_valid():
            ...
            try:
                otp = OTP.objects.filter(public_id=otp_id).first()
            except OTP.DoesNotExist:
            ...
            if otp.is_valid == True:
                ...
                try: 
                    ...
                    user = CustomUser.objects.create_user(phone_number=otp.phone_number)
